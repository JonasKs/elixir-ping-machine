defmodule PingMachine.SubnetManager do
  ...
  def handle_cast({:task, host}, state) do
    task =
      Task.Supervisor.async_nolink(
        # <- We canreference the task supervisor by name
        PingMachine.TaskSupervisor,
        fn ->
          # Pretends to send a ping request by sleeping some time and then
          # randomly selecting a return value for the task. Fail approx 1/3 tasks.
    ...
    # tasks responsed with a successful ping request, and which didn't.
    {:noreply,
  ...
  def handle_info({ref, :ok}, state) do
    ...
    # We don't care about the :DOWN message form the task anymore, so demonitor and flush it
